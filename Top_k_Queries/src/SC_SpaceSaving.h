...
//template<uint64_t sc_memory_in_bytes, uint32_t threshold, uint32_t bucket_num = 96>
template<int sc_memory_in_bytes, uint32_t threshold, uint32_t bucket_num = 96>
...
//    SpaceSaving<ss_capacity> ss;
    SpaceSaving *ss;
public:
    SC_SpaceSaving(int k_size) {
        ss=new SpaceSaving(k_size);
        sc.init_spa(ss);
//        sc.init_spa(&ss);
//        sc.print_basic_info();
    }
