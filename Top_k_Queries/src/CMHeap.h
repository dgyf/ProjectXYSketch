#ifndef STREAMCLASSIFIER_CMHEAP_H
#define STREAMCLASSIFIER_CMHEAP_H
...
public:
    CMHeap(int capacity1, int cm_memory_in_bytes1, int d1) : heap_element_num(0) {
        capacity = capacity1;
        cm_memory_in_bytes = cm_memory_in_bytes1;
        d = d1;
        cm_sketch = (int **) new int *[d1];
...
//            cout <<j<< " ID : " << a[capacity - 1 - j].second << " Frequency ; " << a[capacity - 1 - j].first << endl;
//        }
...
/*
    void get_top_k_with_frequency(uint16_t k, vector<pair<uint32_t, uint32_t>> &result) {
        ...
        for (int j = 0; j < capacity; j++) {
            cout <<j<< " ID : " << heap[capacity - 1 - j].second << " Frequency ; " << heap[capacity - 1 - j].first << endl;
        }

        for (i = 0; i < k && i < capacity; ++i) {
            result[i].first = heap[capacity - 1 - i].second;
            result[i].second = heap[capacity - 1 - i].first;
        }
        for (; i < k; ++i) {
            result[i].first = 0;
            result[i].second = 0;
...
#endif //STREAMCLASSIFIER_CMHEAP_H
