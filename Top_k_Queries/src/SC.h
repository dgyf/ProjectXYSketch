...
#define MAX_HASH_NUM 1000

template<int memory_in_bytes, int bucket_num , int counter_num = 16, int threshold = 240, int l1_ratio = 65>
...
//    uint16_t L2[m2_in_bytes * 8 / 16]; // Layer 2 is organized as counter, one counter consist of 16 bit
    ...
    uint64_t *L1=new uint64_t[m1_in_bytes * 8 / 4 / 16];
    uint16_t *L2=new uint16_t[m2_in_bytes * 8 / 16];
    ...
    void clear_data()
    {
        ...
        for(int i=0;i<m1_in_bytes * 8 / 4 / 16;i++){
            L1[i]=0;
        }
        for(int i=0;i<m2_in_bytes * 8 / 16;i++){
            L2[i]=0;
        }
