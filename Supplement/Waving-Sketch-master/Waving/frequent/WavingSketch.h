...
public:
	...
	void gettopk(int k, std::vector<std::pair<data_type ,count_type >>& ans){
        //std::vector<nod> ans(k);
        int c_max=BUCKET_NUM * slot_num;
        std::vector<std::pair<data_type ,count_type >> temp(c_max);
        int ctt=0;
        for(int i=0;i<BUCKET_NUM;i++){
            for(int j=0;j<slot_num;j++){
                temp[ctt].first=buckets[i].items[j];
                temp[ctt].second=buckets[i].counters[j];
                //std::cout<<"The id : "<<buckets[i].items[j]<<"  The fre: "<<buckets[i].items[j]<<std::endl;
                ctt++;
            }
        }
        sort(temp.begin(),temp.end(),cmp11);
        for(int i=0;i<k;i++){
            ans[i].first=temp[i].first;
            ans[i].second=temp[i].second;
            //std::cout<<"The id : "<<temp[i].first<<"  The fre: "<<temp[i].second<<std::endl;
        }
	}



    static bool cmp11(std::pair<data_type ,count_type > a, std::pair<data_type ,count_type > b) {
