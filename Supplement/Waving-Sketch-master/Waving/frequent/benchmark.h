...
void BenchCmp(int fid) {
	...
	if (fid==0)
		//items=read_skipping_data("../dataset/demo.dat", &cnt);
        //items=read_skipping_data("../demo.dat", &cnt);
    items=read_skipping_data("/home/yqliu/data/kosarak.dat", &cnt);
	else if (fid==1)
        items=read_data("/home/yqliu/data/kosarak.dat", sizeof(data_type), &cnt);
		//items=read_data("/home/yqliu/data/webdocs.dat", sizeof(data_type), &cnt);
    //items=read_data("../dataset/syn.dat", sizeof(data_type), &cnt);
	else if (fid==2)
		items=read_data("../dataset/CAIDA2018.dat", sizeof(data_type), &cnt);
	else if (fid==3)
