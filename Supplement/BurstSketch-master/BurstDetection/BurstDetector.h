...
class ScreenLayer
{
	...
	uint32_t query_scr(uint64_t flow_id){
	    uint32_t ans=0;
        for(int i = 0; i < hash_num; i++)
        {
            int z = bobhash[i]->run((char *)&flow_id, 8) % size;
...
class Log // Stage 2
{
	...
	uint32_t query_log(uint64_t flow_id, int timep){
	    uint32_t ans=0;
        int z = bobhash->run((char *)&flow_id, 8) % size;
        for(int j = 0; j < bucket_size; j++)
        {
...
class BurstDetector // BurstSketch
{
	...
	uint32_t Query(uint64_t key, int timep){
	    uint32_t ans=log.query_log(key,timep);
	    if(ans==0){
	        ans=screen_layer.query_scr(key);
	    }
        return ans;
	}
};
